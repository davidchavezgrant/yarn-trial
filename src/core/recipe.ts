/**
 * Recipes — task-level knowledge an agent writes for the agents that come after it.
 *
 ...
 * panel close. A compiled procedure has all of that and is unusable as knowledge: it is a frozen
 ...
 *
 * Writing the recipe at `done()` would have been simpler and is wrong twice over. It adds a
 ...
 * instead of the brand default. A recipe harvested from one of those would teach every future
...
/**
 * Where a promoted recipe lives: keyed by (app, BACKEND, task).
 *
 * Task, because a recipe for "change the cursor type" must not be found by a run doing
 * "create a two-scene script" — `taskHash` is shared with procedure.ts rather than reimplemented,
 ...
 * the ax and cdp passes name the same surfaces and controls differently, and a recipe names
 * both. Without this axis `ax-recipe` and `cdp-recipe` resolve to ONE file, the
 * second promote overwrites the first, and one arm silently grounds on the other backend's
 * write-up. Nothing downstream would catch it: provenance reads "recipe" either way.
 */
export type RecipeLineage = "grounded" | "ungrounded";
 ...
 *
 * A recipe distilled from an appmap-grounded run presupposes the 40-minute sweep — you cannot
 * conclude the sweep is replaceable from an artifact that requires it to exist. A recipe
 ...
 */
export const recipeFileFor = (dir: string, slug: string, task: string, backend?: string, lineage: RecipeLineage = "grounded"): string =>
	path.join(dir, `${slug}${backend ? `.${backend}` : ""}.${taskHash(task)}${lineage === "ungrounded" ? ".ungrounded" : ""}.recipe.md`);

/** What a run knew, from its own log. "none" provenance is the ungrounded lineage. */
export const lineageOf = (run: HarvestSource): RecipeLineage => (!run.grounding?.provenance || run.grounding.provenance === "none" ? "ungrounded" : "grounded");

/**
 * Why this run may not become a recipe, or undefined if it may.
 *
...
export function harvestRefusal(run: HarvestSource, judge: JudgeVerdict | undefined): string | undefined {
	if (!run.success) return "run did not succeed — a recipe from it would document a route that does not work";
	if (run.hintedPrompt)
		// The same laundering `compileProcedure` refuses. A hinted run was TOLD its route; writing
	...
	if (!judge) return "no judge verdict — run `./run judge <stamp>` first; a recipe is only as trustworthy as the grader that passed it";
	if ((judge.trajectory ?? "").toUpperCase() !== "PASS")
		...
		return `judge returned TRAJECTORY ${judge.trajectory ?? "UNKNOWN"} — only a judged-PASS run may become a recipe`;
	/**
	 * ANY verified channel counts, not just text — and the difference matters more than it looks.
	 *
	 * `compileProcedure` refuses pixel-verified steps, correctly, because a REPLAY has to re-check
	 * an expectation mechanically and "some pixels changed" is not re-checkable. Copying that
	 * gate here was wrong: a recipe is prose for a model, not a machine replay, and the steps
	 ...
	 * canvas work went, producing a confident recipe that omits the part that was hard.
	 */
...
export function routeOf(run: HarvestSource): string {
	return (run.steps ?? [])
		...
		.map((s, i) => {
			...
			// recipe has to say where to act and what should visibly change. Text and geometry
...
export const HARVEST_SYSTEM = [
	"You are writing a short recipe for the next agent that has to do this task in this app.",
	"",
	...
	"",
	"Write the recipe as markdown. Rules:",
	...
	"",
	"Do NOT:",
	"- Invent steps that are not in the route. If the route is short, the recipe is short.",
 ...
 * A recipe with no stamp is treated as curated, exactly as an unstamped appmap is.
 */
export const recipeHeader = (run: HarvestSource, stamp: string, judge: JudgeVerdict): string =>
	`<!-- provenance: recipe | app: ${run.app ?? "unknown"} | task: ${run.task ?? ""} | from: ${stamp} | judge: ${judge.trajectory ?? "?"} | backend: ${run.backend ?? "?"} | lineage: ${lineageOf(run)} -->\n\n`;

/** Write a harvested recipe to a path, creating the directory. Returns the path. */
export function writeRecipe(file: string, body: string): string {
