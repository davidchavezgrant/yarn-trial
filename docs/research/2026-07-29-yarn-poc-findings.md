...
>    hand-written — including task-specific procedures added after watching runs fail at the
>    very tasks measured here (Notion Calendar's timezone entry is a verbatim procedure naming
...
>    exploration output. Those files now live in `docs/procedures/` (a legitimate tier — it IS
>    the procedure direction we propose — but not evidence about the explore→appmap pipeline).
...
is handed over for free. It is retained only because it upper-bounds what procedure compilation
would buy (see below); it must never be quoted as a run result.

### The much bigger lever: procedures
...
17**. The appmap tells the agent *where things are*; the procedure tells it *exactly how to do
this task*, and the second is worth ~3× the first.

That is the empirical case for **procedure compilation**: after a task succeeds once, store the
verified action sequence and replay it, calling the model only when a verification step
fails. Grounding makes the first run cheaper; procedures make every subsequent run nearly free.
...
Only procedure replay removes it: a replayed happy path is 0 model calls.
...
`docs/curated/` for hand-curated notes. Correction note 1 above is why — task-specific
procedures had been hand-added to the appmaps that "grounded" runs were measured against.)
...
a complete verified procedure, so the model emits the known action without reasoning about what
should change.
...
curated map carried as a procedure), **strict verification** (no vacuous passes; checks must
...
- **Procedure replay** — the main lever on per-run cost and determinism.
