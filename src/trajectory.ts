 ...
 * the exact shape `readTrajectory()` in src/track.ts consumes. On the CDP backend there is
 ...
 * snapshot boxes share one CSS-pixel space (scale:"css" — see src/cdp.ts), so click points,
