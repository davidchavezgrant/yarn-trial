 ...
 * that round trip with no quoting at all — see `encodeSpec` in src/remote/ssh.ts, which is
 * the other end of this contract.
 */

const USAGE = `usage: runnerctl <status|submit|logs|stop|apps|job|doctor|grant|restart|signin|liveview|ready> [options]
...
const METHODS = new Set(["status", "submit", "logs", "stop", "apps", "job", "doctor", "grant", "restart", "signin", "liveview", "ready"]);
