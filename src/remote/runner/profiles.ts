...
/** The operator whose data is live for `app` on this Mac, or undefined when nobody owns it yet. */
export function currentOwner(root: string, app: string): string | undefined {
	return readOwners(root)[appSlug(app)];
}

/** Whether `operator` owns the live copy of `app` here — the export-side guard against tarring someone else's session. */
export function ownsLive(root: string, app: string, operator: string): boolean {
	return currentOwner(root, app) === sanitise(operator);
}

/** The path-segment form of an operator name, exported so the vault keys a bundle exactly as the store does. */
export function sanitiseOperator(operator: string): string {
	return sanitise(operator);
}

export type InstallAction =
	/** No parked bundle existed for the operator, so nothing was installed — the run will sign in. */
	| "no-bundle"
	/** The operator already owned the live copy here; local state was left untouched. */
	| "skipped-owned"
	/** The operator's parked bundle was made live, parking any other operator's session first. */
	| "installed";
...
export interface ProfileInstall {
	...
	operator: string;
	previousOwner?: string;
	/** Home-relative paths parked under the previous (other) operator. */
	parked: string[];
	/** Home-relative paths made live from the operator's parked bundle. */
	restored: string[];
}
...
/**
 * Make an operator's ALREADY-PARKED bundle the live session, parking whoever is there.
 *
 * This is the checkout-side counterpart to `swapProfile`. The difference that justifies a second
 * function: `swapProfile` restores a parked profile only when someone ELSE owns the live data
 * (its "swapped" branch), and leaves the live data alone otherwise. A bundle pushed from the
 * vault must go live on a box regardless of who — or no one — currently owns the app, so this
 * always restores from the operator's parked store. The one case it deliberately does NOT touch
 * is when the operator already owns the live copy: a run that just finished there may hold a
 * fresher session than the vault, and the readiness probe, not this function, decides whether the
 * local session still works.
 *
 * The restore is a COPY, not a move: the parked bundle is kept so a job that lands here later
 * behind another operator's run (the queue-contention case) can be restored again by the ordinary
 * `swapProfile`. Every other property — quit-before-move ordering, parking the other operator's
 * session under their own name, traversal-refusal on manifest paths — is `swapProfile`'s, for the
 * same reasons documented there.
 */
export async function installProfile(opts: InstallOptions): Promise<ProfileInstall> {
	const home = opts.home ?? os.homedir();
	const root = opts.root ?? profilesRoot();
	const slug = appSlug(opts.app);
	const operator = sanitise(opts.operator);
	...
	if (!stored) return { action: "no-bundle", app: opts.app, operator, parked: [], restored: [] };
	if (previousOwner === operator) return { action: "skipped-owned", app: opts.app, operator, previousOwner, parked: [], restored: [] };

	await (opts.quit ?? quitApp)(opts.app);

	// Whatever is live belongs to `previousOwner`. A real other operator's session is parked under
	// their name so they never lose it; unowned/factory live is discarded, since the bundle is
	// about to replace it and it belongs to no one.
	const parked: string[] = [];
	const live = capturePaths(id, home);
	if (previousOwner !== undefined) {
		const theirs = profileDir(root, previousOwner, slug);
		fs.mkdirSync(theirs, { recursive: true, mode: 0o700 });
		for (const rel of live) move(path.join(home, rel), path.join(theirs, rel));
		const outgoing: ProfileManifest = {
			app: opts.app,
			operator: previousOwner,
			...(opts.bundleId ? { bundleId: opts.bundleId } : {}),
		...
		for (const rel of live) fs.rmSync(path.join(home, rel), { recursive: true, force: true });
	}
	...
	return { action: "installed", app: opts.app, operator, previousOwner, parked, restored };
}

/** One line for the job log, matching `describeSwap`'s shape. */
export function describeInstall(i: ProfileInstall): string {
	if (i.action === "no-bundle") return `install: no vault bundle for ${i.operator} in ${i.app} — the app will need signing in`;
	if (i.action === "skipped-owned") return `install: ${i.operator} already owns ${i.app} here — kept the local session`;
	const tail = i.previousOwner ? `, parked ${i.parked.length} path(s) for ${i.previousOwner}` : "";

	return `install: ${i.app} restored for ${i.operator}, ${i.restored.length} path(s)${tail}`;
}
