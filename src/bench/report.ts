...
export const reportFileName = (date: string): string => `${date}-backend-grounding-reuse-benchmarks.md`;
...
function replayTable(arms: Arm[], m: Manifest): string[] {
	const header = "| arm | model | flags | done | success | procedure steps | rescued x̄ | calls x̄ | s x̄ |";
	const rows = arms.flatMap((a) =>
		modelPasses(m, a.id).map((model) => {
...
export function renderReport(m: Manifest): string {
	...
	const p3Recipes = phaseArms(3).filter(isTask);
		...
		"Compiles: " +
			(phaseArms(3)
				...
				.map((a) => {
					...
					return `${a.id}: ${e ? (e.procedure ?? e.jobId) : "not run"}`;
				})
				.join("; ") || "none"),
		"",
