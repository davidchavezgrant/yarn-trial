...
explore for ~40 minutes); **curated notes** (written by a human); and a **recipe** (a short
prose write-up harvested from one earlier run that succeeded).

### Procedures vs recipes — who decides at run time
...
A **recipe** is prose that *informs* the model. Nothing in it is executed; the agent reads it
...
Note that last sentence — a caveat about *which* setting was changed. A procedure cannot carry one;
only the tier with a reader can hold advice.

**The measured trade:** procedures went 1/3 with a model on standby to repair broken steps and 0/3
without. Recipes went 3/3 at 848 output tokens, the cheapest grounding we measured.

**Where each belongs:** procedures for re-rendering a demo you have already shot (same customer,
same video, nightly, at no model cost). Recipes for onboarding a new app or task — which is
...
| **recipe, harvested from a run that had no map** | **3/3** | **8** | **848** |
...
2. **Recipe harvesting** — have successful runs write down what worked. Cheapest grounding we
...
2. **Procedures, until they handle variation.** A frozen sequence breaks on two buttons with the same
